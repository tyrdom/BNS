...
init([LSocket]) ->
  {ok, accepting, #state{lsocket = LSocket}}.
...
accepting(_Event, State) ->
  #state{lsocket = LSocket} =State,
  NextState =
  case gen_tcp:accept(LSocket) of
    {ok,CSocket} -> {ok,Pid} = sp_tcp_client:start(CSocket),
    ...
    {error,Reason} -> io:format("tcp_acceptor is error ~p ~n",[Reason]),timeout
